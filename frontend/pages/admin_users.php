...
<<<<<<< HEAD
        ...
        if ($response['success']) {
            ...
            $message = 'Lỗi: ' . ($response['message'] ?? 'Không thể cập nhật trạng thái');
        }
    } elseif ($action === 'update_role') {
        $newRole = $_POST['new_role'];
        // Use backend API to update user role
        $response = makeApiRequest('/users/' . $userId . '/role', 'PUT', [
            'role' => $newRole
        ]);
        ...
        if ($response['success']) {
            ...
            $message = 'Lỗi: ' . ($response['message'] ?? 'Không thể cập nhật vai trò');
        }
    } elseif ($action === 'delete') {
        if ($userId != $_SESSION['user_id']) {
            // Use backend API to delete user
            $response = makeApiRequest('/users/' . $userId, 'DELETE');
