...
// --- 1. CẤU HÌNH PHÂN TRANG (LOGIC MỚI) ---
$page = isset($_GET['page']) ? max(1, (int)$_GET['page']) : 1; // Trang hiện tại, tối thiểu là 1
$limit = 12; // Số sản phẩm mỗi trang (Bạn muốn 10 thì sửa thành 10, nhưng 12 đẹp hơn cho lưới 3/4 cột)
$offset = ($page - 1) * $limit; // Vị trí bắt đầu lấy dữ liệu

// Get filters (Giữ nguyên)
$categoryId = isset($_GET['category']) ? (int)$_GET['category'] : null;
$brandId = isset($_GET['brand']) ? (int)$_GET['brand'] : null;
$search = isset($_GET['search']) ? sanitizeInput($_GET['search']) : '';
$sortBy = isset($_GET['sort']) ? $_GET['sort'] : 'newest';

// --- 2. XÂY DỰNG SQL CƠ BẢN ---
// Tách phần WHERE ra để dùng chung cho cả việc Đếm và Lấy dữ liệu
$whereConditions = "WHERE p.Status = 'active'";
$params = [];
...
if ($brandId) {
    $whereConditions .= " AND p.BrandID = ?";
    $params[] = $brandId;
}

if ($search) {
    $whereConditions .= " AND (p.ProductName LIKE ? OR p.Description LIKE ?)";
    $params[] = "%$search%";
    $params[] = "%$search%";
}

// --- 3. ĐẾM TỔNG SỐ SẢN PHẨM (LOGIC MỚI) ---
// Phải đếm trước để biết có bao nhiêu trang
...
// --- 4. LẤY DỮ LIỆU SẢN PHẨM THEO TRANG (LOGIC MỚI) ---
